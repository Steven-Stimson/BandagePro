...
rem This is the script I use on my Windows 7 virtual machine to build the public release of BandagePro.
rem BandagePro. It produces a directory ready for deployment. It requires that sample_LastGraph is in
...
rem Get the version number from the main.cpp file and replace its dots with underscores.
...
rem Add the necessary libraries so BandagePro can be deployed.
call "%QT_PATH%\%MSVC_VERSION%\bin\windeployqt.exe" Bandage\Bandage.exe

rem Zip BandagePro with the sample graph and clean up.
call "%ZIP_PATH%" a -tzip BandagePro_Windows_32bit_v%VERSION%.zip Bandage\ sample_LastGraph installation.txt
call rmdir Bandage\ /S /Q
